...
namespace CipherEncrytDecrypt
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                StreamReader iFile = File.OpenText(fileName); // opening Occasions.txt text file                 
                while (iFile.EndOfStream == false)    // loop for reading lines one by one from file
                {
                    line += iFile.ReadLine();          // storing one line in the line variable of string type.  
                }
                iFile.Close();
            ...
            catch (Exception ex)                   // Exception handeling code  
            {
                Console.WriteLine("\n ERROR:" + ex.Message); // diplsying error message (if occurs)
            }
